...
function get_category_children($cid)
{
    ...
    $children = Db::name('category')->where([
        'path' => [
            'like',
            "%,{$cid},%"
        ]
...
function get_articles_by_cid($cid, $limit = 10, $where = [], $order = [], $filed = [])
{
    ...
    $article_list = Db::name('article')->where($map)
        ->field($fileds)
        ->order($sort)
        ->limit($limit)
        ->select();
...
function createGoogleAuthenticator($secret = null)
{
    ...
    $qrCodeUrl = $ga->getQRCodeGoogleUrl(Session::get('username') . '%20-%20' . $_SERVER['HTTP_HOST'], $secret);
    $gacode = [
        'secret' => $secret,
        'qrCodeUrl' => $qrCodeUrl
    ];
...
function validateGoogleAuthenticator($secret, $gacode)
{
    $ga = new GoogleAuthenticator();
